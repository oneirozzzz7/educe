      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
          ...
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-semibold text-[15px]" style={{ color: "var(--text)" }}>模型设置</h3>
            <button onClick={onClose} style={{ color: "var(--text-3)" }}><X size={16} /></button>
          </div>
          <label className="block text-[10px] font-medium uppercase tracking-wider mb-1.5" style={{ color: "var(--text-3)" }}>切换模型</label>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={async () => {
              try {
                const r = await fetch(`http://${API_HOST}/api/settings`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: selected }) });
