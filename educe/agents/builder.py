...
from educe.tools.artifacts import ArtifactManager
...
class BuilderAgent(BaseAgent):
    ...
    async def handle(self, message: Message, context: WorkContext) -> AsyncIterator[Message]:
        ...
        # 简单任务跳过 decision 分析——直接构建，不需要讨论
        # 复杂任务才进入分析，但分析可能返回空（无有意义的选择点）
        complexity = context.metadata.get("_task_complexity", "simple")
        skip_analysis = context.metadata.get("_skip_analysis") or complexity == "simple"
        ...
        from educe.core.agentic_loop import AgenticLoop
        ...
        if complexity == "complex":
            ...
            if transcript:
                transcript.add("build", "system", "正在生成步骤计划...")
        ...
        if final_files:
            ...
            if html_files:
                from educe.core.execution_loop import ExecutionLoop
                loop = ExecutionLoop()
    ...
    def _build_prompt(self, message: Message, context: WorkContext) -> str:
        ...
        build_activation = ""
        try:
            from educe.core.activation_engine import BUILD_ACTIVATION, DEFAULT_BUILD_SEED
